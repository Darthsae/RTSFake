...
from ..unit.UnitResourceInstance import UnitResourceInstance
...
@dataclass
class UnitResourceInstanceIndividualData:
    id: str
    presence: Presence
    min: RangeSelect[int]
...
class UnitResourceInstancePredicate:
    def __init__(self, a_resources: list[UnitResourceInstanceIndividualData]) -> None:
        self.resources = a_resources

    def Validate(self, a_resources: dict[str, UnitResourceInstance]) -> bool:
        for resource in self.resources:
            ...
            match resource.presence:
                case Presence.REQUIRED:
                    if not present:
                        return False
                    unitResource: UnitResourceInstance = a_resources[resource.id]
    ...
    def FromDict(cls, a_data: AnyDict) -> Self:
        return cls(
            [UnitResourceInstanceIndividualData.FromDict(data) for data in a_data["resources"]]
        )
