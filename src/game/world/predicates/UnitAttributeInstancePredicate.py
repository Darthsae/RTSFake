...
from ..unit.UnitAttributeInstance import UnitAttributeInstance
...
@dataclass
class UnitAttributeInstanceIndividualData:
    id: str
    presence: Presence
    min: RangeSelect[int]
...
class UnitAttributeInstancePredicate:
    def __init__(self, a_attributes: list[UnitAttributeInstanceIndividualData]) -> None:
        self.attributes = a_attributes

    def Validate(self, a_attributes: dict[str, UnitAttributeInstance]) -> bool:
        for attribute in self.attributes:
            ...
            match attribute.presence:
                case Presence.REQUIRED:
                    if not present:
                        return False
                    unitAttribute: UnitAttributeInstance = a_attributes[attribute.id]
    ...
    def FromDict(cls, a_data: AnyDict) -> Self:
        return cls(
            [UnitAttributeInstanceIndividualData.FromDict(data) for data in a_data["attributes"]]
        )
